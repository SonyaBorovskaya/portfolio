...
<body>
	...
	<div class="ui sidebar menu left vertical inverted">
		...
		<a href="#" class="item subitem">Количество статей авторов</a>
		<a href="#" class="item ui header">
	...
	<div class="container" style="padding: 64px 15px 0px 15px;">
		<h1 class=" ui header" style="text-align: center;">Статьи сотрудников</h1>
	...
	$sql = mysqli_query(
		$connection,
		"SELECT staff_id, concat(staff.s_name,' ',LEFT(staff.f_name,1), '.',LEFT(staff.t_name,1),'.') AS fio, COUNT(*) FROM
		singles INNER JOIN staff ON singles.staff_id=staff.id WHERE date BETWEEN CAST('$n_date' AS DATE) AND CAST('$k_date' AS DATE) GROUP BY
		staff_id;"
	);
